# ICT-BRANDED-APP
<img src="./assets/logo.png" alt="Project Logo" width="200"/>


**Mission :** Provide simple dependable website that will enable our customers to purchase our ICT branded products withiut any effort.

**Vision :** Becoming a number one company in South Africa, with realiable, simple, safe and usable software, providing all the needs our customers.

**Values : :** We value customers need more than money
-We value usability more than appearance
-We value the interests and safety of our customers .
---

## Table of Contents
- [Project Overview](#-corporate colours)
- [The Challenge](#-the-challenge)
- [The Solution](#-the-solution)
- [Key Features](#-key-features)
- [Target Audience](#-target-audience)
- [System Architecture](#-system-architecture)
- [Technologies & Tools](#-technologies--tools)
- [Team](#-team)

---

## Corporate colours
 **-HEX CODE:** #0097B2
 
**-CMYK: ** 100%,15%,0%,30%

**-RGB:** 0,196,204** 

**-HEX CODE:** #FF751F

**-CMYK:** 0%,54%,88%,0%

**-RGB :** 255,177,31

**-HEX CODE:** #101D56

**-CMYK:** 81%,66%,0%,66%

**-RGB:** 16,29,86

---

## The Challenge

**Current Problems:**
- Students in the ICT department are most likely to attend their classes due to frustrations of not knowing what to wear on campus.
- This problem is caused by financial background and awkwardness of repeating their clothing items when attending leading to lowered self esteem and their marks dropping.
- Social isolation .

**Impact of the Problem:**
- Poor academic perfomance, frustration, and significant inconvenience for sense of belonging in the field / ICT industry .
- Missed classes and academic activities .
- Increased stress and anxiety for students .

---

##  The Solution

**ICT-BRANDED Provides:**
- A robust online ecommerce app.
- Real-time display of t-shirts.
- Digital application to buy and order their t-shirts.


**Benefits:**
- Inclusion
- Significantly improved academic record.
- An enhanced, student-specific experience.

---

## Key Features

The ICT-BRANDED app includes a functional web/mobile application powered by a secure database-driven system and an interactive user interface. 

Core functionalities include:
- **Authentication:** Secure user registration and login.
- **Cart Management:** View real-time available products.
- **History Tracking:** Comprehensive cart history logs.
- **Admin Dashboard:** Powerful tools for administrative personnel to manage orders.

---


## Target Audience

**Primary Users:**
- Students .

**Rationale:** 
Students require quick, reliable, and convenient ecommerce application to improve service delivery, accessibility, and operational efficiency.

---

## 🛠️ Technologies & Tools

- **Frontend:** React
- **Backend:** Java, Postman, and JPA
- **Database:** MySQL
- **Tools:** GitHub, Trello, and Figma.

---

##  Team

Presented and developed by:
- Athini Ngquke
- Olwethu Mtwazi
- Sinobawo Nkomo
- Snalo Noni 
- Siphokuhle Nyana

---

*For inquiries or support regarding ICT-BRANDED, please open an issue in the repository.*
